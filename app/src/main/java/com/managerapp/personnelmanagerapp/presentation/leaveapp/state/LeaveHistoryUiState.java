...
public class LeaveHistoryUiState {
    ...
    private final List<LeaveApplicationResponse> leaveApplications;
    ...
    public LeaveHistoryUiState(boolean isLoading,
                               List<LeaveApplicationResponse> leaveApplications,
                               String errorMessage,
                               int approvedCount,
                               int pendingCount,
                               int rejectedCount) {
    ...
    public static LeaveHistoryUiState success(List<LeaveApplicationResponse> leaveApplications) {
    ...
    private static LeaveStats countLeaveStatus(List<LeaveApplicationResponse> applications) {
        int approvedCount = 0, pendingCount = 0, rejectedCount = 0;

        if (applications != null) {
            for (LeaveApplicationResponse application : applications) {
                LeaveStatus status = LeaveStatus.fromString(application.getFormStatusEnum());
                if (status != null) {
                    switch (status) {
                        case APPROVED:
                            approvedCount++;
